...
<script>
import { selectObj, updateObj, customerObj } from "api/addService/myaddService/index";
export default {
  data () {
    return {
      filterText: '',
  ...
  methods: {
    ...
    applyServices () {
      ...
      if (!this.form.customerCode == true) {
        this.$message({
          message: "客户不能为空！",
          type: 'error',
          duration: 2000
      ...
      this.$confirm('确定要保存吗?', '编辑', {
        ...
        this.form.customerCode = this.form.customerCode.substring(this.form.customerCode.indexOf("(") + 1, this.form.customerCode.indexOf(")"))
    ...
    newApplyServices () {
      ...
      this.$confirm('确定要重新申请吗?', '重新申请', {
        ...
        let formApply = {
          ...
          customerCode: this.form.customerCode.substring(this.form.customerCode.indexOf("(") + 1, this.form.customerCode.indexOf(")"))
        }
