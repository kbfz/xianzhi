<template>
  <div class="right-bg-box">
    <el-form :model="serviceRecord"
      ...
      <div class="tablecontent">
          ...
          <el-table-column align="center"
            ...
            <el-table-column align="center"
              ...
              <template scope="scope">
                ...
                <span style="cursor: pointer;"
                      v-else>{{scope.row.actualServicePrice}}</span>
              </template>
            </el-table-column>
            <el-table-column align="center"
                             label="实际完成数量"
            ...
            <el-table-column align="center"
              ...
              <template scope="scope">
                <span v-if="!serviceRecord.operateTime">暂无</span>
                <span style="cursor: pointer;"
                      v-else>{{scope.row.actualTotalPrice}}</span>
              </template>
            </el-table-column>
          </el-table-column>
        </el-table>
        <el-form-item label="申请总价（不含税）："
                      label-width="153px"
                      class="tablebottom">
        ...
        <el-form-item label="优惠："
                      label-width="153px"
                      class="tablebottom">
          <span v-if="!serviceRecord.discountPrice">暂无</span>
          <span v-else>{{serviceRecord.discountPrice}}</span>
        </el-form-item>
        <el-form-item label="客户应付金额："
                      label-width="153px"
                      class="tablebottom redtext">
      ...
      <el-form-item label="备注："
        ...
        <span v-else>{{serviceRecord.payRecord.remark}}</span>
      ...
      <el-form-item label="付款凭证："
        ...
        <span v-if="!serviceRecord.payRecord">暂无</span>
        <img v-else
             :src="serviceRecord.payRecord.payProof"
             class="avatar">
      </el-form-item>
    ...
    <hr class="hrcss">
    <div v-for="item in serviceRecord.serviceRecordList" class="caozuo" :key="item">
