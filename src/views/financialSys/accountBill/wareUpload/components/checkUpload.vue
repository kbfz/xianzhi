        ...
        <div class="app-container-left uploadDialog">
          <div class="filter-container">
            ...
            >
              ...
              <el-table-column min-width="100px" align="center" label="查验费" show-overflow-tooltip>
                <template scope="scope">
          ...
          >
            <el-form :model="form" :rules="rules" ref="form" label-width="90px" :inline="true">
              ...
              <div style="position: relative;float: left">
                <span class="xhText" style="left: 29px">*</span>
                <el-form-item label="查验费" class="marginListAdd">
                  <el-input v-model="form.checkFee" placeholder="查验费" style="width:200px"></el-input>
                </el-form-item>
              </div>
            </el-form>
...
import { getCheckFeeObj } from "@/api/financialSys/customerBill";
import { checkAdd,checkGet,checkUpdate,checkDel } from "api/financialSys/wareupload/index";
...
export default {
  ...
  methods: {
    getList() {
      ...
      getCheckFeeObj(param).then((response) => {
    ...
    updateSubmit() {
      ...
      this.$confirm("确定要保存吗", "提示", {
        ...
        checkUpdate(obj.id, obj).then((res) => {
          if (res.rel) {
            this.$notify({
              title: "成功",
              message: "保存成功",
    ...
    exportSave(event) {
      ...
      this.$http.post(
          "/api/ordersys/customerFinanceBillCheck/importCheck",
          formData,
          config
        )
        .then((res) => {
